...
public class Matrix extends BoundingBox		// function of the domain
	{
		...
		*/
		...
		/**
			...
			Set every pixel such that Mask.val[i][j]=True to black color
			@param Mask = mask to apply
		*/
		public void applyMask(Mask mask) {
			assert ((height==mask.height)&&(width==mask.width)):"Mask and Matrix do not have the same sizes.";
			for(int j=0;j<height;j++)
				for(int i=0;i<width;i++)
					if (mask.val[i][j])
						val[i][j]=new Color(0,0,0);
		}
		/**
